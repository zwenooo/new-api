...
// TODO: when new upstreams/models are enabled, check the pricing here
// 1 === $0.002 / 1K tokens
// 1 === ￥0.014 / 1k tokens
...
func GetCompletionRatio(name string) float64 {
	...
	if ratio, ok := CompletionRatio[rawName]; ok {
		return ratio
	}
...
func FormatMatchingModelName(name string) string {
	name = stripOpenAIReasoningSuffix(name)
...
func stripOpenAIReasoningSuffix(name string) string {
	suffixes := []string{"-xhigh", "-high", "-minimal", "-low", "-medium", "-none"}
	for _, suffix := range suffixes {
		if !strings.HasSuffix(name, suffix) {
			continue
		}
		trimmed := strings.TrimSuffix(name, suffix)
		if strings.HasPrefix(trimmed, "gpt-5") || strings.Contains(trimmed, "codex") {
			return trimmed
		}
	}
	return name
