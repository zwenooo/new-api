...
func DoDownloadRequestWithProxy(originUrl string, proxy string, reason ...string) (resp *http.Response, err error) {
	if system_setting.EnableWorker() {
		...
		if proxy != "" {
			...
			if err != nil {
				return nil, fmt.Errorf("failed to create proxy http client: %v", err)
			}
...
func DoDownloadRequest(originUrl string, reason ...string) (resp *http.Response, err error) {
	return DoDownloadRequestWithProxy(originUrl, "", reason...)
}
