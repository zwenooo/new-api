...
const (
	PayTokenOrderStatusPending = "pending"
	PayTokenOrderStatusSuccess = "success"
	PayTokenOrderStatusFailed  = "failed"
)

const (
	PayTokenPayMethodEpay    = "epay"
	PayTokenPayMethodBalance = "balance"
)

// PayTokenOrder represents a top-up order that converts RMB (fen) to non-expiring token credits.
type PayTokenOrder struct {
	...
	// CreditTokens is the token credits to be credited after payment success (tokens).
	CreditTokens int `json:"credit_tokens" gorm:"type:int;default:0;column:credit_tokens"`

	// Product fields for product-based pay-token system.
...
func (PayTokenOrder) TableName() string {
	return "pay_token_orders"
}

func (o *PayTokenOrder) Insert(tx *gorm.DB) error {
	if o == nil {
	...
	if o.PayMethod != PayTokenPayMethodEpay && o.PayMethod != PayTokenPayMethodBalance {
