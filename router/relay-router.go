...
func SetRelayRouter(router *gin.Engine) {
	...
	// Anthropic-compatible utility endpoints that should NOT depend on channel distribution.
	// (Some clients call these before any model routing happens, e.g. `/compact` flow.)
	relayV1TokenOnlyRouter := router.Group("/v1")
	relayV1TokenOnlyRouter.Use(middleware.SystemPerformanceCheck(), middleware.TokenAuth())
	{
		relayV1TokenOnlyRouter.POST("/messages/count_tokens", controller.CountClaudeMessageTokens)
	}
	...
	playgroundRouter.Use(middleware.SystemPerformanceCheck(), middleware.UserAuth(), middleware.Distribute())
	{
	...
	// Distribute may rewrite UsingGroupId (e.g. from token allowed groups).
	...
	}
	{
		...
		httpRouter.POST("/chat/completions", func(c *gin.Context) {
			if !operation_setting.ChatCompletionsEnabled {
				controller.RelayChatCompletionsDisabled(c)
				return
			}
			controller.Relay(c, types.RelayFormatOpenAI)
		})
	...
	relaySunoRouter.Use(middleware.SystemPerformanceCheck(), middleware.TokenAuth(), middleware.Distribute())
	{
	...
	// Distribute may rewrite UsingGroupId.
