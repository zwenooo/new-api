...
const (
	ErrorTypeNewAPIError     ErrorType = "transfer_api_error"
...
const (
	...
	ErrorCodeUserDailyQuotaExceeded     ErrorCode = "user_daily_quota_exceeded"
...
func (e *NewAPIError) LogMessage() string {
	if e == nil {
		return ""
	}
	if e.Err == nil {
	...
	errStr := e.Err.Error()
	var masked maskedError
	if errors.As(e.Err, &masked) && masked.err != nil {
		errStr = masked.err.Error()
	}
	return errStr
}
...
func (e *NewAPIError) ToOpenAIError() OpenAIError {
	...
	default:
		openAIErrorType := string(e.errorType)
		if e.errorType == ErrorTypeNewAPIError {
			openAIErrorType = mapStatusToOpenAIErrorType(e.StatusCode)
		}
		result = OpenAIError{
			Message: e.Error(),
...
func mapStatusToOpenAIErrorType(statusCode int) string {
	...
	default:
		return string(ErrorTypeNewAPIError)
	}
}
...
func (e *NewAPIError) ToClaudeError() ClaudeError {
	...
	case ErrorTypeOpenAIError:
		if openAIError, ok := e.RelayError.(OpenAIError); ok {
			typ := mapOpenAIErrorTypeToAnthropic(openAIError.Type)
			if typ == "api_error" {
				if t := mapStatusToAnthropicErrorType(e.StatusCode); t != "" {
					typ = t
				}
			}
			result = ClaudeError{Type: typ, Message: e.Error()}
		}
	case ErrorTypeClaudeError:
		if claudeError, ok := e.RelayError.(ClaudeError); ok {
			result = claudeError
	...
	default:
		typ := mapStatusToAnthropicErrorType(e.StatusCode)
		if typ == "" {
			typ = "api_error"
		}
		result = ClaudeError{Type: typ, Message: e.Error()}
...
func ErrOptionWithHideErrMsg(replaceStr string) NewAPIErrorOptions {
	return func(e *NewAPIError) {
		...
		e.Err = maskedError{msg: replaceStr, err: e.Err}
	}
}
