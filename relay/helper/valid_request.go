...
func normalizeValidatedModelName(model string) string {
...
func GetAndValidAudioRequest(c *gin.Context, relayMode int) (*dto.AudioRequest, error) {
	...
	audioRequest.Model = normalizeValidatedModelName(audioRequest.Model)
	switch relayMode {
	...
	default:
		...
		if audioRequest.Model == "" {
			audioRequest.Model = normalizeValidatedModelName(formData.Get("model"))
		}
...
func GetAndValidateEmbeddingRequest(c *gin.Context, relayMode int) (*dto.EmbeddingRequest, error) {
	...
	embeddingRequest.Model = normalizeValidatedModelName(embeddingRequest.Model)
	...
	if relayMode == relayconstant.RelayModeEmbeddings && embeddingRequest.Model == "" {
		embeddingRequest.Model = normalizeValidatedModelName(c.Param("model"))
	}
...
func GetAndValidateResponsesRequest(c *gin.Context) (*dto.OpenAIResponsesRequest, error) {
	...
	request.Model = normalizeValidatedModelName(request.Model)
	if request.Model == "" {
		return nil, errors.New("model is required")
	}
...
func GetAndValidateResponsesWebSocketRequestBytes(body []byte) (*dto.OpenAIResponsesRequest, []byte, error) {
	trimmed := bytes.TrimSpace(body)
	if len(trimmed) == 0 {
		return nil, nil, errors.New("websocket request body is empty")
	}
	...
	default:
		return nil, nil, fmt.Errorf("unsupported websocket request type: %s", strings.TrimSpace(envelope.Type))
	}
	...
	if strings.HasPrefix(strings.TrimSpace(request.PreviousResponseID), "msg_") {
		return nil, nil, errors.New("previous_response_id must be a response.id (resp_*), not a message id")
	}
...
func GetAndValidOpenAIImageRequest(c *gin.Context, relayMode int) (*dto.ImageRequest, error) {
	...
	case relayconstant.RelayModeImagesEdits:
		if strings.Contains(c.Request.Header.Get("Content-Type"), "multipart/form-data") {
			...
			imageRequest.Model = normalizeValidatedModelName(formData.Get("model"))
	...
	default:
		...
		imageRequest.Model = normalizeValidatedModelName(imageRequest.Model)
...
func GetAndValidateClaudeRequest(c *gin.Context) (textRequest *dto.ClaudeRequest, err error) {
	...
	textRequest.Model = normalizeValidatedModelName(textRequest.Model)
...
func GetAndValidateTextRequest(c *gin.Context, relayMode int) (*dto.GeneralOpenAIRequest, error) {
	...
	if relayMode == relayconstant.RelayModeEmbeddings && textRequest.Model == "" {
		textRequest.Model = normalizeValidatedModelName(c.Param("model"))
	}
