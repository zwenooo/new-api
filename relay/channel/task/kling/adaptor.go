...
func (a *TaskAdaptor) DoResponse(c *gin.Context, resp *http.Response, info *relaycommon.RelayInfo) (taskID string, taskData []byte, taskErr *dto.TaskError) {
	...
	kResp.TaskId = upstreamTaskID
	clientResponseBody := responseBody
	if info.TaskRelayInfo != nil && strings.TrimSpace(info.TaskRelayInfo.PublicTaskID) != "" {
		kResp.TaskId = info.TaskRelayInfo.PublicTaskID
		kResp.Data.TaskId = info.TaskRelayInfo.PublicTaskID
		clientResponseBody, err = json.Marshal(kResp)
		if err != nil {
...
func (a *TaskAdaptor) convertToRequestPayload(req *relaycommon.TaskSubmitReq, info *relaycommon.RelayInfo) (*requestPayload, error) {
	...
	r := requestPayload{
		...
		ModelName:      modelName,
		Model:          modelName, // Keep consistent with model_name, double writing improves compatibility
		CfgScale:       0.5,
		StaticMask:     "",
