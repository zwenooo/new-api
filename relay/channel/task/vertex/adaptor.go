...
func (a *TaskAdaptor) DoResponse(c *gin.Context, resp *http.Response, info *relaycommon.RelayInfo) (taskID string, taskData []byte, taskErr *dto.TaskError) {
	...
	taskIDForClient := localID
	if info.TaskRelayInfo != nil && strings.TrimSpace(info.TaskRelayInfo.PublicTaskID) != "" {
		taskIDForClient = info.TaskRelayInfo.PublicTaskID
	}
	clientResponseBody, err := json.Marshal(gin.H{"task_id": taskIDForClient})
	if err != nil {
		return "", nil, service.TaskErrorWrapper(err, "marshal_response_failed", http.StatusInternalServerError)
	}
	return localID, clientResponseBody, nil
}
