...
func RequestOpenAI2ClaudeComplete(textRequest dto.GeneralOpenAIRequest) *dto.ClaudeRequest {
	...
	if claudeRequest.MaxTokensToSample == nil {
		claudeRequest.MaxTokensToSample = common.GetPointer[uint](4096)
	}
	prompt := ""
...
func RequestOpenAI2ClaudeMessage(c *gin.Context, textRequest dto.GeneralOpenAIRequest) (*dto.ClaudeRequest, error) {
	...
	if claudeRequest.MaxTokens == nil || *claudeRequest.MaxTokens == 0 {
		claudeRequest.MaxTokens = common.GetPointer(uint(model_setting.GetClaudeSettings().GetDefaultMaxTokens(textRequest.Model)))
	}
...
func FormatClaudeResponseInfo(requestMode int, claudeResponse *dto.ClaudeResponse, oaiResponse *dto.ChatCompletionsStreamResponse, claudeInfo *ClaudeResponseInfo, originModelName string, maskUpstreamModel bool) bool {
	if claudeInfo == nil {
		return false
	}
	...
	if oaiResponse != nil {
		...
		if maskUpstreamModel {
			oaiResponse.Model = originModelName
		} else {
...
func HandleStreamResponseData(c *gin.Context, info *relaycommon.RelayInfo, claudeInfo *ClaudeResponseInfo, data string, requestMode int) *types.NewAPIError {
	...
	if info.RelayFormat == types.RelayFormatClaude {
		FormatClaudeResponseInfo(requestMode, &claudeResponse, nil, claudeInfo, info.OriginModelName, false)
		...
		if !FormatClaudeResponseInfo(requestMode, &claudeResponse, response, claudeInfo, info.OriginModelName, !helper.IsAdminUser(c)) {
			return nil
		}
...
func HandleStreamFinalResponse(c *gin.Context, info *relaycommon.RelayInfo, claudeInfo *ClaudeResponseInfo, requestMode int) {
	...
	if info.RelayFormat == types.RelayFormatClaude {
		...
		if info.ShouldIncludeUsage {
			modelForClient := claudeInfo.Model
			if !helper.IsAdminUser(c) {
				modelForClient = info.OriginModelName
			}
			response := helper.GenerateFinalUsageResponse(claudeInfo.ResponseId, claudeInfo.Created, modelForClient, *claudeInfo.Usage)
			err := helper.ObjectData(c, response)
			if err != nil {
