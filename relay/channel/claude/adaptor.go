...
func shouldUseClaudeFullPassthrough(info *relaycommon.RelayInfo) bool {
	if info == nil {
		return false
	}
	if info.ApiType != constant.APITypeAnthropic || info.RelayFormat != types.RelayFormatClaude {
		return false
	}
...
func (a *Adaptor) GetRequestURL(info *relaycommon.RelayInfo) (string, error) {
	if shouldUseClaudeFullPassthrough(info) {
...
func (a *Adaptor) SetupRequestHeader(c *gin.Context, req *http.Header, info *relaycommon.RelayInfo) error {
	if shouldUseClaudeFullPassthrough(info) {
		channel.SetupPassThroughRequestHeader(c, req, "Authorization", "X-Api-Key")
