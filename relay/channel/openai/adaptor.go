...
import (
	...
	cx2cc "one-api/relay/compat/cx2cc"
...
func (a *Adaptor) ConvertClaudeRequest(c *gin.Context, info *relaycommon.RelayInfo, request *dto.ClaudeRequest) (any, error) {
	if common.GetContextKeyBool(c, constant.ContextKeyChannelMessagesToResponsesCompat) {
		responsesReq, err := cx2cc.PrepareChannelMessagesToResponsesCompatRequest(c, info, request)
		if err != nil {
			return nil, err
		}
...
func (a *Adaptor) GetRequestURL(info *relaycommon.RelayInfo) (string, error) {
	if info.RelayMode == relayconstant.RelayModeRealtime ||
		(info.RelayMode == relayconstant.RelayModeResponses && info.ClientWs != nil) {
	...
	case constant.ChannelTypeAzure:
		...
		if info.RelayMode == relayconstant.RelayModeResponses {
			isCompact := strings.HasPrefix(strings.Split(info.RequestURLPath, "?")[0], "/v1/responses/compact")
...
func (a *Adaptor) SetupRequestHeader(c *gin.Context, header *http.Header, info *relaycommon.RelayInfo) error {
	...
	if info.ChannelType == constant.ChannelTypeOpenRouter {
		header.Set("HTTP-Referer", "https://github.com/Calcium-Ion/transfer-api")
		header.Set("X-Title", "Transfer API")
	}
	if info.RelayMode == relayconstant.RelayModeResponses && relaycommon.IsDirectCodexResponsesUpstream(info) {
		...
		} else if strings.TrimSpace(header.Get("Accept")) == "" {
...
func (a *Adaptor) DoResponse(c *gin.Context, resp *http.Response, info *relaycommon.RelayInfo) (usage any, err *types.NewAPIError) {
	...
	case relayconstant.RelayModeResponses:
		if info.RelayFormat == types.RelayFormatClaude {
			if info.IsStream {
				usage, err = OaiResponsesToClaudeStreamHandler(c, info, resp)
			} else {
				usage, err = OaiResponsesToClaudeHandler(c, info, resp)
			}
		} else if info.ClientWs != nil && info.TargetWs != nil {
			usage, err = OpenaiResponsesWebSocketHandler(c, info)
		} else {
			if info.IsStream {
