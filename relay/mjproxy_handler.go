...
func coverMidjourneyTaskDto(c *gin.Context, originTask *model.Midjourney) (midjourneyTask dto.MidjourneyDto) {
	...
	midjourneyTask.Quota = originTask.Quota
	midjourneyTask.VisibleQuota = originTask.VisibleQuota
	midjourneyTask.CostQuota = originTask.CostQuota
...
func RelaySwapFace(c *gin.Context, info *relaycommon.RelayInfo) *dto.MidjourneyResponse {
	...
	if userQuota-priceData.Quota < 0 {
		allow := false
		if info.UsingGroupId > 0 {
			_, _, totalUnlimited, _, capErr := model.GetUserSubscriptionCapacityForGroup(info.UserId, info.UsingGroupId)
	...
	// 订阅/令牌当日日限前置校验，避免先返回成功、后续扣费失败
	if err := service.EnsureDailyQuotaForExpected(info, priceData.Quota); err != nil {
		desc := "user_daily_quota_exceeded"
		if errors.Is(err, model.ErrTokenDailyQuotaExceeded) {
			desc = "token_daily_quota_exceeded"
		}
		return &dto.MidjourneyResponse{Code: 4, Description: desc}
	}
	...
	defer func() {
		if mjResp.StatusCode == 200 && mjResp.Response.Code == 1 {
			err := service.PostConsumeQuota(info, priceData.Quota, priceData.Quota, 0, true)
			if err != nil {
				// 将后置扣费错误改写为任务失败，避免消费统计与日志不一致
				if errors.Is(err, model.ErrTokenDailyQuotaExceeded) {
					mjResp.Response.Code = 4
					mjResp.Response.Description = "token_daily_quota_exceeded"
				} else if errors.Is(err, model.ErrUserDailyQuotaExceeded) {
					mjResp.Response.Code = 4
					mjResp.Response.Description = "user_daily_quota_exceeded"
				} else {
					mjResp.Response.Code = 4
					mjResp.Response.Description = "update_data_error"
				}
				return
			}

			tokenName := c.GetString("token_name")
			logContent := fmt.Sprintf("模型固定价格 %.2f，分组倍率 %.2f，操作 %s", priceData.ModelPrice, priceData.GroupRatioInfo.EffectiveGroupRatio, constant.MjActionSwapFace)
			...
			model.UpdateUserUsageAndRequestCount(info.UserId, priceData.Quota, priceData.VisibleQuota, priceData.CostQuota)
			model.UpdateChannelUsageQuotas(info.ChannelId, priceData.Quota, priceData.VisibleQuota, priceData.CostQuota)
		}
	}()
...
func RelayMidjourneySubmit(c *gin.Context, relayInfo *relaycommon.RelayInfo) *dto.MidjourneyResponse {
	...
	noBilling, err := model.IsUserEligibleForNoBillingGroup(relayInfo.UserId, relayInfo.UsingGroupId)
	if err != nil {
		return &dto.MidjourneyResponse{
			Code:        4,
			Description: err.Error(),
	...
	if noBilling {
		relayInfo.QuotaBucket = model.UserQuotaBucketFree
	}
	...
	defer func() {
		if consumeQuota && midjResponseWithStatus.StatusCode == 200 {
			err := service.PostConsumeQuota(relayInfo, priceData.Quota, priceData.Quota, 0, true)
			if err != nil {
				common.SysLog("error consuming token remain quota: " + err.Error())
			}
			tokenName := c.GetString("token_name")
			logContent := fmt.Sprintf("模型固定价格 %.2f，分组倍率 %.2f，操作 %s，ID %s", priceData.ModelPrice, priceData.GroupRatioInfo.EffectiveGroupRatio, midjRequest.Action, midjResponse.Result)
			other := service.GenerateMjOtherInfo(priceData)
			...
			model.UpdateUserUsageAndRequestCount(relayInfo.UserId, priceData.Quota, priceData.VisibleQuota, priceData.CostQuota)
			model.UpdateChannelUsageQuotas(relayInfo.ChannelId, priceData.Quota, priceData.VisibleQuota, priceData.CostQuota)
		}
	}()
