...
const EditPrefillGroupModal = ({
  ...
  const handleSubmit = async (values) => {
    ...
    try {
      ...
      } else if (values.type === 'proxy') {
        const proxy = String(values.items || '').trim();
        submitData.items = proxy ? [proxy] : [];
      } else {
          ...
          initValues={{
            ...
            items: (() => {
              try {
                ...
                if (editingGroup?.type === 'proxy') {
                  if (Array.isArray(editingGroup?.items)) {
                    return editingGroup.items?.[0] || '';
                  }
                  return typeof editingGroup?.items === 'string'
                    ? editingGroup.items
                    : '';
                ...
                if (editingGroup?.type === 'endpoint') return '';
                if (editingGroup?.type === 'proxy') return '';
                return [];
              }
            })(),
          }}
                ...
                <Col span={24}>
                  {selectedType === 'endpoint' ? (
                    ...
                    <Form.Input
                      ...
                      showClear
                      rules={[
                        { required: true, message: t('请输入代理地址') },
                      ]}
                      extraText={t('每个代理组只能配置 1 个代理地址')}
                    />
                  ) : (
                    <Form.TagInput
                      field='items'
