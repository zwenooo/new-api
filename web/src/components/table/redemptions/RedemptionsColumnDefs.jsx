...
const renderMode = (record, t) => {
  const mode = inferRedemptionMode(record);
  const modeMap = {
    activation: { color: 'blue', text: '激活码' },
    free: { color: 'grey', text: '自由额度' },
    subscription: { color: 'green', text: '订阅额度' },
    tokens: { color: 'indigo', text: 'Token 订阅' },
    request: { color: 'purple', text: '次数订阅' },
    payg: { color: 'orange', text: '按量付费' },
    pay_request: { color: 'red', text: '按次付费' },
    pay_token: { color: 'violet', text: '按 Token 付费' },
    xiaotuan: { color: 'cyan', text: '小团订阅' },
  };
  const config = modeMap[mode] || { color: 'black', text: mode };
  return (
...
export const getRedemptionsColumns = ({
  ...
  return [
    ...
    },
    {
      ...
      dataIndex: 'mode',
      render: (text, record) => {
        return <div>{renderMode(record, t)}</div>;
      },
    },
    {
      title: t('额度'),
      ...
      render: (_, record) => {
        ...
        const quota = Number(record?.quota ?? 0) || 0;
        let label = '-';
        if (mode === 'tokens' || mode === 'pay_token') {
          label = quota <= 0 ? t('无限') : `${renderNumber(quota)} tokens`;
        } else if (mode === 'request' || mode === 'pay_request') {
          label = quota <= 0 ? t('无限') : `${renderNumber(quota)} ${t('次')}`;
        } else {
    ...
    },
    {
      ...
      dataIndex: 'price_fen',
      render: (text) => {
        const priceFen = parseInt(text, 10) || 0;
        if (priceFen <= 0) return <div>-</div>;
        return (
          <div>
            <Tag color='grey' shape='circle'>
              {renderCnyFen(priceFen)}
            </Tag>
          </div>
        );
      },
          ...
          <Space>
            ...
            <Button
              ...
              onClick={() => {
                setEditingRedemption({
                  ...record,
                  mode: inferRedemptionMode(record),
                });
                setShowEdit(true);
              }}
              disabled={
                record.status !== REDEMPTION_STATUS.UNUSED ||
                inferRedemptionMode(record) !== 'subscription'
              }
            >
              {t('编辑')}
            </Button>
