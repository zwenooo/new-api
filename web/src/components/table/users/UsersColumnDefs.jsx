    ...
    <Popover content={popoverContent} position='top'>
      <Tag color='white' shape='circle'>
        <div className='flex flex-col items-end'>
          <span className='text-xs leading-none'>{`${renderQuota(
            remain,
          )} / ${renderQuota(total)}`}</span>
          <Progress
            percent={percent}
  ...
  return (
    <div>
      <Space spacing={1}>
        ...
        <Tag color='white' shape='circle' className='!text-xs'>
          {t('收益')}: {renderCnyFen(record.aff_history_quota)}
        </Tag>
        <Tag color='white' shape='circle' className='!text-xs'>
          {record.inviter_id === 0
            ? t('无邀请人')
...
export const getUsersColumns = ({
  ...
  return [
    ...
    },
    {
      ...
      render: (_, record) => {
        const subLimit = parseInt(record.subscription_daily_limit) || 0;
        const subUsed = parseInt(record.subscription_daily_used) || 0;
        const unlimited = Boolean(record.subscription_daily_limit_unlimited);

        // 若存在任一订阅不限日限，则整体视为「每日额度不限」
        if (unlimited || subLimit === 0) {
          return t('每日额度不限');
        }

        const remain = Math.max(0, subLimit - subUsed);
        return `${renderQuotaToUSD(remain)} / ${renderQuotaToUSD(subLimit)}`;
      },
    },
    {
      title: t('到期日期'),
      dataIndex: 'redeem_quota_expire_at',
      render: (ts) => {
        const v = parseInt(ts) || 0;
        if (v <= 0) return t('不限时');
        return <>{timestamp2string(v).slice(0, 10)}</>;
      },
    },
    {
      title: t('用户分组'),
      dataIndex: 'user_group_label',
      render: (text) => {
