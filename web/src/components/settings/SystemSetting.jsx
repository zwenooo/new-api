...
const SystemSetting = () => {
  ...
  let [inputs, setInputs] = useState({
    ...
    ClawBoxActivationEnabled: '',
    ClawBoxRegisterEnabled: '',
    ClawBoxProductModeEnabled: '',
    ClawBoxProductId: '',
    ClawBoxInitialShrimp: '',
    SMTPSSLEnabled: '',
  ...
  const [baseUrlsDraft, setBaseUrlsDraft] = useState('');
    ...
    if (success) {
      ...
      data.forEach((item) => {
	        switch (item.key) {
	          ...
	          case 'BaseUrls':
	            try {
	              const urls = item.value ? JSON.parse(item.value) : [];
	              setBaseUrls(Array.isArray(urls) ? urls : []);
	            } catch (e) {
	              setBaseUrls([]);
	            }
	            break;
  ...
  const submitServerAddress = async () => {
    ...
    if (ServerAddress.endsWith('/v1')) {
      showError(
        t('服务器地址不要包含 /v1，请将 /v1 配置在下方「基址」中，例如：https://yourdomain.com/v1'),
      );
      return;
    }
  ...
  const submitBaseUrls = async () => {
    ...
    const draft = String(baseUrlsDraft || '').trim();
    const nextBaseUrls = draft ? [...baseUrls, draft] : baseUrls;

    const normalized = nextBaseUrls.map((it) => String(it || '').trim()).filter((it) => it);
    const unique = Array.from(new Set(normalized));

    await updateOptions([{ key: 'BaseUrls', value: JSON.stringify(unique) }]);
    setBaseUrlsDraft('');
  };
                ...
                <Form.Section text={t('通用设置')}>
                  ...
                  >
                    <Col xs={24} sm={24} md={24} lg={24} xl={24}>
	                  ...
	                  >
	                    <Col xs={24} sm={24} md={24} lg={24} xl={24}>
	                      <Text strong>{t('基址')}</Text>
		                      <Text
		                        type='secondary'
		                        style={{ display: 'block', marginBottom: 8 }}
		                      >
		                        {t(
		                          '可配置多个基址。支持 「说明:|地址」 或 「说明，地址/说明,地址」 格式（如：三网优化:|https://xxx.com 或 三网优化，https://xxx.com），复制按钮将只复制分隔符后面的地址。',
		                        )}
		                      </Text>
		                      <TagInput
		                        value={baseUrls}
		                        onChange={(value) => setBaseUrls(value)}
		                        onInputChange={(value) => setBaseUrlsDraft(value)}
		                        placeholder={t(
		                          '输入基址后回车，如：https://yourdomain.com/v1 或 三网优化，https://yourdomain.com/v1',
		                        )}
		                        style={{ width: '100%' }}
		                      />
		                    </Col>
		                  </Row>
	                  <Button style={{ marginTop: 8 }} onClick={submitBaseUrls}>
	                    {t('更新基址')}
	                  </Button>
	                </Form.Section>
	              </Card>
              ...
              <Card>
                <Form.Section text={t('代理设置')}>
                  <Text>
                    （支持{' '}
                    <a
                      href='https://github.com/Calcium-Ion/transfer-api-worker'
                      target='_blank'
                      rel='noreferrer'
                    >
                      transfer-api-worker
