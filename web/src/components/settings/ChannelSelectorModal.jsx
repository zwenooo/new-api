...
const ChannelSelectorModal = forwardRef(
  (
    ...
    const getSourceType = (record) =>
      record?._originalData?.source_type || '';
    ...
    const isOfficialChannel = (record) => {
      ...
      return (
        getSourceType(record) === 'official_ratio_preset' ||
        id === -100 ||
    ...
    const isOpenRouterChannel = (record) =>
      getSourceType(record) === 'openrouter_models';
    ...
    useEffect(() => {
      ...
      const sorted = [...matched].sort((a, b) => {
        const wa = isOfficialChannel(a) ? 0 : isOpenRouterChannel(a) ? 1 : 2;
        const wb = isOfficialChannel(b) ? 0 : isOpenRouterChannel(b) ? 1 : 2;
        return wa - wb;
      });
    ...
    const renderEndpointCell = (text, record) => {
      ...
      const currentEndpoint =
        ...
        record?._originalData?.default_endpoint ||
        '';

      if (isOpenRouterChannel(record)) {
        return (
          <Tag color='blue' shape='circle' type='light'>
            {currentEndpoint || '/api/v1/models'}
          </Tag>
        );
      }
