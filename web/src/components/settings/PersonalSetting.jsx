...
const PersonalSetting = () => {
  ...
  useEffect(() => {
    if (userState?.user?.setting) {
      ...
      setNotificationSettings({
        ...
        acceptUnsetModelRatioModel: isAdminUser
          ? !!settings.accept_unset_model_ratio_model
          : false,
        recordIpLog: isAdminUser
          ? typeof settings.record_ip_log === 'boolean'
            ? settings.record_ip_log
            : true
          : true,
      });
    }
  }, [userState?.user?.setting, isAdminUser]);
  ...
  const changeAvatar = async () => {
    ...
    try {
      const res = await API.put('/api/user/avatar');
      const { success, message, data } = res.data;
      if (!success) {
        showError(message);
        return;
  ...
  const saveNotificationSettings = async () => {
    try {
      const res = await API.put('/api/user/setting', {
        ...
        accept_unset_model_ratio_model: isAdminUser
          ? notificationSettings.acceptUnsetModelRatioModel
          : false,
        record_ip_log: isAdminUser ? notificationSettings.recordIpLog : true,
      });
  ...
  const showOtherSettingsCard = isAdminUser;
    ...
    <ConsolePage hideHeader>
      ...
      <div
        className={`grid grid-cols-1 ${
          showOtherSettingsCard ? 'xl:grid-cols-2' : ''
