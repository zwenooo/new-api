...
const CardTable = ({
  ...
  const MobileRowCard = ({ record, index }) => {
    ...
    const expandedKeys = Array.isArray(tableProps?.expandedRowKeys)
      ? tableProps.expandedRowKeys
      : null;
    const childrenExpanded = expandedKeys
      ? expandedKeys.includes(rowKeyVal)
      : showChildren;

    const toggleChildren = (next) => {
      if (typeof tableProps?.onExpand === 'function') {
        tableProps.onExpand(next, record);
      }
      if (!expandedKeys) {
        setShowChildren(next);
      }
    };
        ...
        {hasChildren ? (
          <>
            <Button
              ...
              icon={childrenExpanded ? <IconChevronUp /> : <IconChevronDown />}
              onClick={(e) => {
                e.stopPropagation();
                toggleChildren(!childrenExpanded);
              }}
            >
              {childrenExpanded ? t('收起') : t('子渠道')} ({children.length})
            </Button>
            <Collapsible isOpen={childrenExpanded} keepDOM>
              <div
                className='pt-2 flex flex-col gap-2 pl-2 border-l border-dashed'
                style={{ borderColor: 'var(--semi-color-border)' }}
              >
                {children.map((child, childIdx) => (
                  <MobileRowCard
                    key={getRowKey(child, childIdx)}
                    record={child}
                    index={childIdx}
                  />
                ))}
