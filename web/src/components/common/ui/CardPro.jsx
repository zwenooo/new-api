...
const CardPro = ({
  ...
  const footerContent = paginationArea ? (
    <div
      className={`flex w-full ${isMobile ? 'justify-center' : 'justify-between items-center'}`}
    >
      {paginationArea}
    </div>
  ) : null;
  ...
  return (
    <section
      className={`table-scroll-card cardpro ${bordered ? '' : 'cardpro-borderless'} ${className}`.trim()}
      style={style}
      {...props}
    >
      {headerContent && <div className='cardpro-header'>{headerContent}</div>}
      <div className='cardpro-body'>{children}</div>
      {footerContent && <div className='cardpro-footer'>{footerContent}</div>}
    </section>
  );
};
