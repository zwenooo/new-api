...
function BlockCode({ className, children }) {
  const ref = useRef(null);
...
function CustomCode({ className, children }) {
  const classNameText = clsx(className);
  const text = String(children ?? '');
  const isInline =
    !classNameText.includes('hljs') &&
    !classNameText.includes('language-') &&
    !text.includes('\n');
...
function _MarkdownContent(props) {
  ...
  return (
    <ReactMarkdown
      remarkPlugins={
        enableBreaks ? [RemarkMath, RemarkGfm, RemarkBreaks] : [RemarkMath, RemarkGfm]
      }
