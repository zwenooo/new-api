...
export function isAdmin() {
  return hasActualAdminRole();
}

export function isRoot() {
  return hasActualRootRole();
}
...
export const calculateModelPrice = ({
  ...
  if (selectedGroup === 'all' || usedGroupRatio === undefined) {
    ...
    if (
      Array.isArray(record.enable_group_ids) &&
      record.enable_group_ids.length > 0
    ) {
      record.enable_group_ids.forEach((g) => {
        const raw = Number(g);
        const gid = Number.isFinite(raw) ? Math.floor(raw) : 0;
        if (gid <= 0) return;
        const r = groupRatio[gid];
  ...
  if (record.quota_type === 0) {
    // 按量计费
    const baseInputRatioPriceUSD = record.model_ratio * 2;
    const baseCompletionRatioPriceUSD =
      record.model_ratio * record.completion_ratio * 2;
    const inputRatioPriceUSD = baseInputRatioPriceUSD * usedGroupRatio;
    const completionRatioPriceUSD =
      baseCompletionRatioPriceUSD * usedGroupRatio;
    ...
    return {
      ...
      originalInputPrice: formatTokenPrice(baseInputRatioPriceUSD),
      originalCompletionPrice: formatTokenPrice(baseCompletionRatioPriceUSD),
      unitLabel,
      isPerToken: true,
      usedGroup,
      usedGroupRatio,
  ...
  if (record.quota_type === 1) {
    ...
    return {
      price: displayVal,
      originalPrice: displayPrice(basePriceUSD),
      isPerToken: false,
      usedGroup,
      usedGroupRatio,
    };
...
export const formatPriceInfo = (priceData, t) => {
  const showOriginalPrice = Number(priceData?.usedGroupRatio) !== 1;
  ...
  return (
    <div className='flex flex-col gap-0.5'>
      ...
      {showOriginalPrice && priceData.originalPrice ? (
        <div className='flex items-center gap-2'>
          <span
            style={{
              color: 'var(--semi-color-text-3)',
