...
const mergeSidebarConfig = (baseConfig, overrideConfig) => {
  const merged = { ...baseConfig, ...overrideConfig };

  Object.keys(baseConfig).forEach((sectionKey) => {
    if (typeof baseConfig[sectionKey] !== 'object' || !baseConfig[sectionKey]) {
      return;
    }
    const overrideSection =
      typeof overrideConfig?.[sectionKey] === 'object' &&
      overrideConfig[sectionKey]
        ? overrideConfig[sectionKey]
        : {};
    merged[sectionKey] = {
      ...baseConfig[sectionKey],
      ...overrideSection,
    };
  });
  ...
  const adminConfig = useMemo(() => {
    if (statusState?.status?.SidebarModulesAdmin) {
      try {
        ...
        return mergeSidebarConfig(defaultAdminConfig, config);
      } catch (error) {
  ...
  const loadUserConfig = async () => {
    try {
      ...
      if (res.data?.success && res.data?.data) {
        setPermissionConfig(res.data.data.permissions?.sidebar_modules || null);
      } else {
