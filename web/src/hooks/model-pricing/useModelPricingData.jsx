...
export const useModelPricingData = () => {
  ...
  // Pricing page is user-facing: only show user_selectable groups, even for admin accounts.
  // `/api/pricing.usable_group` is the authoritative "user selectable" group set (enabled && user_selectable).
  const usableGroupIdSet = useMemo(() => {
    const set = new Set();
    const m = usableGroup && typeof usableGroup === 'object' ? usableGroup : {};
    Object.keys(m).forEach((k) => {
      const raw = Number(k);
      const id = Number.isFinite(raw) ? Math.floor(raw) : 0;
      if (id > 0) set.add(id);
    });
    return set;
  }, [usableGroup]);

  const userSelectableGroups = useMemo(() => {
    const list = Array.isArray(rawUserGroups) ? rawUserGroups : [];
    if (usableGroupIdSet.size === 0) return [];
    return list.filter((g) => {
      const rawId = Number(g?.id ?? 0);
      const id = Number.isFinite(rawId) ? Math.floor(rawId) : 0;
      if (id <= 0) return false;
      return usableGroupIdSet.has(id);
    });
  }, [rawUserGroups, usableGroupIdSet]);

  const groupLabelById = useMemo(() => {
    const map = {};
    const list = Array.isArray(userSelectableGroups) ? userSelectableGroups : [];
    list.forEach((g) => {
      const rawId = Number(g?.id ?? 0);
      const id = Number.isFinite(rawId) ? Math.floor(rawId) : 0;
      if (id <= 0) return;
  ...
  const filteredModels = useMemo(() => {
    ...
    // 分组筛选
    if (filterGroup !== 'all') {
      const raw = Number(filterGroup);
      const groupId = Number.isFinite(raw) ? Math.floor(raw) : 0;
      if (groupId <= 0) {
        return [];
      }
      result = result.filter((model) => {
        const enableIds = Array.isArray(model.enable_group_ids)
          ? model.enable_group_ids
          : [];
        return enableIds.some((g) => Number(g) === groupId);
      });
    }
  ...
  const loadUserSelectableGroups = async () => {
    try {
      const res = await API.get('/api/user/self/groups');
      const { success, data } = res.data;
      if (success) {
        setRawUserGroups(Array.isArray(data) ? data : []);
        return;
      }
      setRawUserGroups([]);
    } catch (e) {
      console.error('Failed to load user selectable groups:', e);
      setRawUserGroups([]);
    }
  };

  const refresh = async () => {
    await Promise.all([loadPricing(), loadUserSelectableGroups()]);
  };
    ...
    if (group === 'all') {
      ...
      showInfo(
        t('当前查看的分组为：{{group}}，倍率为：{{ratio}}', {
          group: groupLabelById[group] || t('未知分组'),
