...
export const useRedemptionsData = () => {
  ...
  // Batch disable redemption codes
  const batchDisableRedemptions = async () => {
    if (selectedKeys.length === 0) {
      showError(t('请至少选择一个兑换码！'));
      return;
    }

    const candidates = selectedKeys.filter(
      (rec) =>
        rec.status === REDEMPTION_STATUS.UNUSED && !isRedemptionExpired(rec),
    );

    if (candidates.length === 0) {
      showError(t('所选兑换码均不可禁用'));
      return;
    }
