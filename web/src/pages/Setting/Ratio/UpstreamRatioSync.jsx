...
import { DEFAULT_ENDPOINT, OPENROUTER_MODELS_ENDPOINT } from '../../../constants';
...
export default function UpstreamRatioSync(props) {
  ...
  const [openRouterPriceSyncToken, setOpenRouterPriceSyncToken] = useState('');
  const isMobile = useIsMobile();
      ...
      if (res.data.success) {
        ...
        setChannelEndpoints((prev) => {
          ...
          transferData.forEach((channel) => {
            ...
            const sourceType = channel._originalData?.source_type || '';
            const defaultEndpoint =
              channel._originalData?.default_endpoint ||
              (sourceType === 'openrouter_models'
                ? OPENROUTER_MODELS_ENDPOINT
                : DEFAULT_ENDPOINT);
            if (!merged[id]) {
              merged[id] = defaultEndpoint;
            }
          });
          return merged;
        });
  ...
  const fetchRatiosFromChannels = async (channelList) => {
    ...
    const upstreams = channelList.map((ch) => ({
      ...
      endpoint:
        channelEndpoints[ch.id] ||
        ch.default_endpoint ||
        (ch.source_type === 'openrouter_models'
          ? OPENROUTER_MODELS_ENDPOINT
          : DEFAULT_ENDPOINT),
  ...
  const applySync = async () => {
    ...
    const formatRatioDescription = (ratios, model, isLocal = false) =>
      [
        `${t('模型倍率')} : ${isLocal ? (ratios.ModelRatio[model] ?? '-') : (ratios['model_ratio'] ?? '-')}`,
        `${t('补全倍率')} : ${isLocal ? (ratios.CompletionRatio[model] ?? '-') : (ratios['completion_ratio'] ?? '-')}`,
        `${t('缓存倍率')} : ${isLocal ? (ratios.CacheRatio[model] ?? '-') : (ratios['cache_ratio'] ?? '-')}`,
        `${t('缓存写入倍率')} : ${isLocal ? (ratios.CreateCacheRatio[model] ?? '-') : (ratios['create_cache_ratio'] ?? '-')}`,
      ].join('\n');
      ...
      if (localCat && localCat !== newCat) {
        const currentDesc =
          localCat === 'price'
            ...
            : formatRatioDescription(currentRatios, model, true);

        let newDesc = '';
        if (newCat === 'price') {
          newDesc = `${t('固定价格')} : ${ratios['model_price']}`;
        } else {
          newDesc = formatRatioDescription(ratios, model, false);
        }
  ...
  const submitOpenRouterPriceSyncToken = async () => {
    const token = openRouterPriceSyncToken.trim();
    if (!token) {
      return;
    }
  ...
  const renderHeader = () => (
    <div className='flex flex-col w-full'>
      <div className='flex flex-col md:flex-row items-end gap-2 w-full'>
        <Input
          value={openRouterPriceSyncToken}
          onChange={setOpenRouterPriceSyncToken}
          placeholder={t('OpenRouter 价格同步 Token')}
          type='password'
          mode='password'
        ...
        />
        <Button
          onClick={submitOpenRouterPriceSyncToken}
          loading={saveTokenLoading}
          disabled={!openRouterPriceSyncToken.trim()}
