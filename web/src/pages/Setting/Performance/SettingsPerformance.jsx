...
export default function SettingsPerformance(props) {
  ...
  function onSubmit() {
    ...
    const requestQueue = updateArray.map((item) => {
      const value = typeof inputs[item.key] === 'boolean' ? String(inputs[item.key]) : String(inputs[item.key]);
    ...
    Promise.all(requestQueue)
      .then((res) => {
        if ((requestQueue.length === 1 && res.includes(undefined)) || (requestQueue.length > 1 && res.includes(undefined))) {
  ...
  useEffect(() => {
    ...
    Object.keys(props.options || {}).forEach((key) => {
      if (!(key in nextInputs)) {
        return;
      }
      if (typeof nextInputs[key] === 'boolean') {
        nextInputs[key] = props.options[key] === 'true' || props.options[key] === true;
      } else if (typeof nextInputs[key] === 'number') {
        nextInputs[key] = parseInt(props.options[key], 10) || nextInputs[key];
      } else {
      ...
      <Spin spinning={loading}>
        <Form values={inputs} getFormApi={(formApi) => { refForm.current = formApi; }} style={{ marginBottom: 15 }}>
          ...
          <Form.Section text={t('系统性能监控')}>
            <Banner
              type='info'
              description={t('启用性能监控后，当系统资源使用率超过设定阈值时，将拒绝新的 Relay 请求 (/v1, /v1beta 等)，以保护系统稳定性。')}
              style={{ marginBottom: 16 }}
            />
            <Row gutter={16}>
              <Col xs={24} sm={12} md={6} lg={6} xl={6}>
                <Form.Switch
                  ...
                  checkedText='｜'
                  uncheckedText='〇'
                  onChange={handleFieldChange('performance_setting.monitor_enabled')}
                />
              </Col>
              <Col xs={24} sm={12} md={6} lg={6} xl={6}>
                <Form.InputNumber
                  ...
                  min={0}
                  max={100}
                  onChange={handleFieldChange('performance_setting.monitor_cpu_threshold')}
              ...
              <Col xs={24} sm={12} md={6} lg={6} xl={6}>
                <Form.InputNumber
                  ...
                  min={0}
                  max={100}
                  onChange={handleFieldChange('performance_setting.monitor_memory_threshold')}
                  disabled={!inputs['performance_setting.monitor_enabled']}
                />
              </Col>
              <Col xs={24} sm={12} md={6} lg={6} xl={6}>
                <Form.InputNumber
                  ...
                  min={0}
                  max={100}
                  onChange={handleFieldChange('performance_setting.monitor_disk_threshold')}
                  disabled={!inputs['performance_setting.monitor_enabled']}
                />
              </Col>
            </Row>
                ...
                <Col xs={24} md={12} style={{ display: 'flex' }}>
                  <div style={{ padding: 16, background: 'var(--semi-color-fill-0)', borderRadius: 8, flex: 1, display: 'flex', flexDirection: 'column' }}>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}>
                      ...
                      <Text type='tertiary'>{t('活跃文件')}: {stats.cache_stats.active_disk_files}</Text>
                    </div>
                    <div style={{ marginTop: 'auto', display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                      <Tag color='blue'>{t('磁盘命中')}: {stats.cache_stats.disk_cache_hits}</Tag>
                      <Tag color='cyan'>{t('预留占用')}: {formatBytes(stats.cache_stats.reserved_disk_usage_bytes)}</Tag>
                    </div>
                  </div>
                </Col>
                <Col xs={24} md={12} style={{ display: 'flex' }}>
                  ...
                  <Col span={24}>
                    <div style={{ padding: 16, background: 'var(--semi-color-fill-0)', borderRadius: 8 }}>
                      ...
                      {stats.disk_space_info.free < inputs['performance_setting.disk_cache_max_size_mb'] * 1024 * 1024 && (
                        <Banner type='warning' description={t('磁盘可用空间小于缓存最大总量设置')} style={{ marginTop: 8 }} />
                      )}
                    </div>
                  </Col>
                </Row>
