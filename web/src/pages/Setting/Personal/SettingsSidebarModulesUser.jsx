  ...
  const generateDefaultConfig = () => {
    ...
    if (isSidebarSectionAllowed('console')) {
      defaultConfig.console = {
        ...
        stomp_king: isSidebarModuleAllowed('console', 'stomp_king'),
    ...
    if (isSidebarSectionAllowed('personal')) {
      defaultConfig.personal = {
        ...
        service_status: isSidebarModuleAllowed('personal', 'service_status'),
      };
    }
    ...
    if (isSidebarSectionAllowed('admin')) {
      defaultConfig.admin = {
        ...
        product_management: isSidebarModuleAllowed(
          'admin',
          'product_management',
        ),
        order: isSidebarModuleAllowed('admin', 'order'),
    ...
    const loadConfigs = async () => {
      try {
        ...
        if (statusState?.status?.SidebarModulesAdmin) {
          ...
          setAdminConfig(normalizedAdminConf);
          console.log('加载管理员边栏配置:', normalizedAdminConf);
        }
        ...
        if (userRes.data.success && userRes.data.data.sidebar_modules) {
          ...
          // 确保用户配置也经过权限过滤
          const mergedUserConf = generateDefaultConfig();
          Object.keys(userConf).forEach((sectionKey) => {
            if (
              userConf[sectionKey] &&
              typeof userConf[sectionKey] === 'object' &&
              !Array.isArray(userConf[sectionKey])
            ) {
            ...
            mergedUserConf[sectionKey] = userConf[sectionKey];
          });
  ...
  const sectionConfigs = [
    ...
    },
    {
      ...
      modules: [
        ...
        { key: 'log', title: t('调用日志'), description: t('API使用记录') },
        {
          key: 'stomp_king',
          title: t('谁是蹬王'),
          description: t('当日 Token 消耗排行'),
        },
        {
          key: 'midjourney',
          title: t('绘图日志'),
    ...
    },
    {
      ...
      modules: [
        ...
        { key: 'user', title: t('用户管理'), description: t('用户账户管理') },
        {
          ...
          title: t('商品管理'),
          description: t('商品与付费规则管理'),
        },
        {
          key: 'order',
          title: t('订单管理'),
          description: t('订单查询与审计'),
        },
        {
          key: 'setting',
          title: t('系统设置'),
