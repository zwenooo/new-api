...
func ListModels(c *gin.Context, modelType int) {
	...
	allowedModelSet := make(map[string]struct{})
	if len(tokenAllowedGroupIDs) > 0 {
		for _, gid := range tokenAllowedGroupIDs {
			if gid <= 0 {
				continue
			}
			for _, m := range model.GetGroupEnabledModels(gid) {
				allowedModelSet[m] = struct{}{}
			}
		}
	}
	...
	if modelLimitEnable {
		...
		if ok {
			if v, ok := s.(map[string]bool); ok {
				tokenModelLimit = v
			}
		}
		for allowModel, _ := range tokenModelLimit {
			if len(allowedModelSet) > 0 {
				if _, ok := allowedModelSet[allowModel]; !ok {
					continue
				}
			}
		...
		var models []string
		if len(allowedModelSet) > 0 {
			for modelName := range allowedModelSet {
			...
			models = model.GetGroupEnabledModels(groupID)
		}
