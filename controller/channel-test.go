...
func recordChannelTestErrorLog(c *gin.Context, channel *model.Channel, err *types.NewAPIError, useTimeSeconds int) {
	...
	if usingGroupID <= 0 {
		usingGroupID = common.GetContextKeyInt(c, constant.ContextKeyDefaultModelGroupId)
	}
	group := ""
	if usingGroupID > 0 {
		group = fmt.Sprintf("%d", usingGroupID)
	}
	...
	model.RecordErrorLog(c, userId, channel.Id, modelName, tokenName, content, c.GetInt("token_id"), useTimeSeconds, false, group, other)
}
...
func testChannel(channel *model.Channel, testModel string) testResult {
	...
	defer middleware.ReleaseCurrentChannelConcurrencySlot(c)
	request := buildTestRequest(c, testModel)
	...
	other := service.GenerateTextOtherInfo(c, info, priceData.ModelRatio, priceData.GroupRatioInfo.GroupRatio, priceData.CompletionRatio,
		usage.PromptTokensDetails.CachedTokens, priceData.CacheRatio, priceData.ModelPrice, priceData.GroupRatioInfo.GroupSpecialRatio, priceData.GroupRatioInfo.BaseMultiplierApplied)
...
func buildTestRequest(c *gin.Context, model string) *dto.GeneralOpenAIRequest {
	...
	if channelSetting, ok := common.GetContextKeyType[dto.ChannelSettings](c, constant.ContextKeyChannelSetting); ok {
		if !channelSetting.TestEnableMaxTokens {
...
func testAllChannels(notify bool) error {
	...
	gopool.Go(func() {
		...
		for _, channel := range channels {
			...
			if newAPIError != nil {
				shouldBanChannel, _, _ = service.ShouldDisableChannel(channel.Type, result.newAPIError)
			}
			...
			willProcessChannelError := isChannelEnabled && shouldBanChannel && channel.GetAutoBan()
			if newAPIError != nil && !willProcessChannelError {
				recordChannelTestErrorLog(result.context, channel, newAPIError, int(float64(milliseconds)/1000.0))
			}

			// disable channel
			if willProcessChannelError {
				_ = processChannelError(result.context, nil, *types.NewChannelError(channel.Id, channel.Type, channel.Name, channel.ChannelInfo.IsMultiKey, common.GetContextKeyString(result.context, constant.ContextKeyChannelKey), channel.GetAutoBan()), newAPIError)
			}
