...
func enrichModels(models []*model.Model) {
	...
	for name, indices := range exactIdx {
		...
		for _, idx := range indices {
			...
			mm.EnableGroupIds = model.GetModelEnableGroupIDs(mm.ModelName)
	...
	groupSetByIdx := make(map[int]map[int]struct{})
	...
	for _, p := range pricings {
		for _, idx := range ruleIndices {
			...
			if gs == nil {
				gs = make(map[int]struct{})
				groupSetByIdx[idx] = gs
			}
			for _, g := range p.EnableGroupIds {
				gs[g] = struct{}{}
			}
	...
	for _, idx := range ruleIndices {
		...
		if gs, ok := groupSetByIdx[idx]; ok {
			groups := make([]int, 0, len(gs))
			for g := range gs {
				groups = append(groups, g)
			}
			sort.Ints(groups)
			mm.EnableGroupIds = groups
		}
