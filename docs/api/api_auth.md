...
1. **请求头中的 `Authorization` 字段**
    ...
    **兼容说明：** 也兼容以下写法（部分客户端会加 `Bearer` 前缀）：

    ```
    Authorization: Bearer <your_access_token>
    ```

2. **请求头中的 `Transfer-Api-User` 字段**

    将用户 ID 放置于 HTTP 请求头部的 `Transfer-Api-User` 字段中，格式如下：

    ```
    Transfer-Api-User: <your_user_id>
    ```

    其中 `<your_user_id>` 需要替换为实际的用户 ID。

    **兼容说明：** 也兼容 `New-Api-User` 作为别名请求头：
...
*   **必须同时提供 `Authorization` 和 `Transfer-Api-User`（或 `New-Api-User`）两个请求头才能通过鉴权。**
...
*   如果 `Transfer-Api-User/New-Api-User` 中的用户 ID 与 Access Token 不匹配，则会返回 `401 Unauthorized` 错误，并提示“无权进行此操作，与登录用户不匹配，请重新登录”。
*   如果没有提供 `Transfer-Api-User/New-Api-User` 请求头，则会返回 `401 Unauthorized` 错误，并提示“无权进行此操作，未提供 Transfer-Api-User/New-Api-User”。
*   如果 `Transfer-Api-User/New-Api-User` 请求头格式错误，则会返回 `401 Unauthorized` 错误，并提示“无权进行此操作，Transfer-Api-User/New-Api-User 格式错误”。
