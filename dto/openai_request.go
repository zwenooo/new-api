...
func (r *OpenAIResponsesRequest) ParseInput() []MediaInput {
	...
	extractUrl := func(value any) string {
		switch v := value.(type) {
		case string:
			return v
		case map[string]any:
		...
		var parseParts func(partsAny any, dst *[]MediaInput)
		parseParts = func(partsAny any, dst *[]MediaInput) {
			if partsAny == nil {
				return
			}
		switch parts := partsAny.(type) {
		case string:
			*dst = append(*dst, MediaInput{Type: "input_text", Text: parts})
		case map[string]any:
			typeVal, _ := parts["type"].(string)
			switch typeVal {
			case "input_text", "text", "output_text":
				text, _ := parts["text"].(string)
				*dst = append(*dst, MediaInput{Type: "input_text", Text: text})
			case "input_image":
				detail, _ := parts["detail"].(string)
				// Some variants put detail inside image_url object.
				if detail == "" {
					if m, ok := parts["image_url"].(map[string]any); ok {
						detail, _ = m["detail"].(string)
					}
				}
				*dst = append(*dst, MediaInput{Type: "input_image", ImageUrl: extractUrl(parts["image_url"]), Detail: detail})
			case "input_file":
				*dst = append(*dst, MediaInput{Type: "input_file", FileUrl: extractUrl(parts["file_url"])})
			}
		case []any:
			for _, itemAny := range parts {
				parseParts(itemAny, dst)
			}
		}
	}

	parseMessage := func(message map[string]any, dst *[]MediaInput) {
		if message == nil {
			return
		}
		parseParts(message["content"], dst)
	}
	...
	case "array":
		...
		for _, itemAny := range array {
			...
			if !ok {
				// Allow shorthand array of strings.
				if str, ok := itemAny.(string); ok {
					inputs = append(inputs, MediaInput{Type: "input_text", Text: str})
				}
				continue
			}

				typeVal, hasType := item["type"].(string)
				if hasType {
					switch typeVal {
					case "input_text", "input_image", "input_file":
						parseParts(item, &inputs)
						continue
					case "message":
						parseMessage(item, &inputs)
						continue
					}
				}

			// Handle the canonical Responses API shape:
			// input: [{ role: "...", content: [{type:"input_text", text:"..."}] }]
			if _, hasRole := item["role"]; hasRole && item["content"] != nil {
				parseMessage(item, &inputs)
				continue
			}
		}
		case "object":
			...
			_ = common.Unmarshal(r.Input, &obj)
			if obj != nil {
				if typeVal, ok := obj["type"].(string); ok && typeVal == "message" {
					parseMessage(obj, &inputs)
					return inputs
				}
				if typeVal, ok := obj["type"].(string); ok && (typeVal == "input_text" || typeVal == "input_image" || typeVal == "input_file" || typeVal == "text") {
					parseParts(obj, &inputs)
					return inputs
				}
				if _, hasRole := obj["role"]; hasRole && obj["content"] != nil {
					parseMessage(obj, &inputs)
					return inputs
				}
		}
